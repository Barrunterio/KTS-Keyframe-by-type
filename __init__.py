# (GNU GPL) <2024> <Barrunterio>" Developed for Blender 3.2, Tested on 4.1
# This program is free software: you can redistribute it and/or modify it, WITHOUT ANY WARRANTY. 

bl_info = {
    "name": "KTS_keyframe_by_type_selector",
    "author": "Barrunterio",
    "version": (0, 0, 1),
    "blender": (4, 0, 0),
    "location": "Dopesheet, Timeline",
    "description": "Adds operations for select keyframes by type",
    "warning": "",
    "wiki_url": "",
    "category": "",
}
...
####
class BRT_KTS_Properties(bpy.types.PropertyGroup):
...
def BRT_KTS_get_range():
    tbtool = bpy.context.scene.BRT_KTS_prop
...
def BRT_KTS_select_keyframes_by_type(context):
    tbtool = bpy.context.scene.BRT_KTS_prop

    if context.space_data.ui_mode in ['DOPESHEET','ACTION']:
        act_keyframe_range = BRT_KTS_get_range()
...
class BRT_KTS_SELECTOR(bpy.types.Operator):
    ...
    def execute(self, context):
        BRT_KTS_select_keyframes_by_type(context)
        return {"FINISHED"}
    
def BRT_KTS_panel(self, context):
        tbtool = bpy.context.scene.BRT_KTS_prop
        layout = self.layout
        box = layout.box()
        row = box.row(align=True)
        BRT_KTS_context_index = 0
        if tbtool.keyframe_range == 'SELECTION':
            BRT_KTS_context_index = 0
        elif tbtool.keyframe_range == 'COLLECTION':
            BRT_KTS_context_index = 1
        elif tbtool.keyframe_range == 'SCENE':
            BRT_KTS_context_index = 2
        elif tbtool.keyframe_range == 'ALL':
            BRT_KTS_context_index = 3      
        BRT_KTS_type_index = 0
        if tbtool.keyframe_type == 'KEYFRAME':
            BRT_KTS_type_index = 0
        elif tbtool.keyframe_type == 'BREAKDOWN':
            BRT_KTS_type_index = 1
        elif tbtool.keyframe_type == 'MOVING_HOLD':
            BRT_KTS_type_index = 2
        elif tbtool.keyframe_type == 'EXTREME':
            BRT_KTS_type_index = 3       
        elif tbtool.keyframe_type == 'JITTER':
            BRT_KTS_type_index = 4                           
        if tbtool.keyframe_type_context:
            ...
            row.prop(tbtool, "keyframe_type",text="",icon=tbtool.keyframe_type_icon[BRT_KTS_type_index])      
        row.separator()         
        if context.space_data.ui_mode in ['DOPESHEET','ACTION']:
            if tbtool.keyframe_range == 'COLLECTION':
                ...
                row.prop(tbtool, "keyframe_range",text="",icon=tbtool.keyframe_context_icon[BRT_KTS_context_index])
            else:
                row.prop(tbtool, "keyframe_range",text="",icon=tbtool.keyframe_context_icon[BRT_KTS_context_index])
...
class TB_PT_KBS(bpy.types.Panel):
    ...
    def draw (self,context):
        BRT_KTS_panel(self, context)

classes = (BRT_KTS_Properties,
    BRT_KTS_SELECTOR,
    TB_PT_KBS,
    )
...
def register():
    for cls in classes:
        ...
        bpy.types.Scene.BRT_KTS_prop = bpy.props.PointerProperty(type= BRT_KTS_Properties)   

def unregister():
 #CLASSES
    for cls in classes:
        bpy.utils.unregister_class(cls)
        bpy.types.Scene.BRT_KTS_prop
